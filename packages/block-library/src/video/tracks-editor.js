...
function TrackList( { tracks, onEditPress } ) {
	if ( tracks.length === 0 ) {
		return (
			<span className="block-library-video-tracks-editor__tracks-informative-message">
				{ __(
	...
	return (
		<Dropdown
			...
			renderContent={ ( {} ) => {
				...
				return (
					<>
						<MenuGroup label={ __( 'Tracks' ) }>
							<TrackList
								tracks={ tracks }
								onEditPress={ setTrackBeingEdited }
							/>
						</MenuGroup>
						<MenuGroup label={ __( 'Add tracks' ) }>
							<NavigableMenu>
								<MediaUpload
									onSelect={ ( { url } ) => {
										onChange( [
											...tracks,
											{ src: url, label: 'Track X' },
										] );
									} }
